...
class CallingDataHandler():
    ...
    def init_orders_status(self, line):
        ...
        if self.calling_data[line]["IsCombine"]:
            ...
            self.orders_status["currentOrderName"] = list(self.calling_data[line]["Orders"].keys())[0]
        for productCode, product in self.calling_data[line]["Orders"][self.orders_status["currentOrderName"]].items():
            if productCode != "_id":
    ...
    def check_AllOrderFull(self, line):
        if self.calling_data[line]["IsCombine"]:
            ...
            if is_current_order_full:
                idx = list(self.calling_data[line]["Orders"].keys()).index(self.orders_status["currentOrderName"])
                if idx < len(self.calling_data[line]["Orders"].keys()) - 1:
                    self.orders_status["currentOrderName"] = list(self.calling_data[line]["Orders"].keys())[idx+1]
    ...
    def counting(self, line, productCode):
        ...
        if self.calling_data[line]["IsCombine"]:
            ...
            if self.calling_data[line]["Orders"][self.orders_status["currentOrderName"]][productCode]["CurrentQuantity"] < self.calling_data[line]["Orders"][self.orders_status["currentOrderName"]][productCode]["ProductCount"]:
                self.calling_data[line]["Orders"][self.orders_status["currentOrderName"]][productCode]["CurrentQuantity"] += 1
                isUpdate = True
                self.dbmanager.update_OrderData_counting(productCode, [self.orders_status["currentOrderName"]], dataHandler.calling_data[line])
                if self.calling_data[line]["Orders"][self.orders_status["currentOrderName"]][productCode]["CurrentQuantity"] == self.calling_data[line]["Orders"][self.orders_status["currentOrderName"]][productCode]["ProductCount"]:
                    self.orders_status["product"][productCode] = True
            else:
                self.orders_status["product"][productCode] = True
...
def countingData():
    ...
    if data['imageBase64'] == "":
        ...
        dbmanager.insert_ConfuseData(data, dataHandler.calling_data[line]['PlateNumber'], dataHandler.orders_status['currentOrderName'], list(dataHandler.orders_status['product'].keys()))
    return jsonify(dataHandler.calling_data)
